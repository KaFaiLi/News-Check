...
class GoogleNewsScraper:
    def __init__(self, max_articles_per_keyword, location="US", language="en"):
        ...
        self.search_url_base = (
            "https://www.google.com/search"  # Changed from news.google.com
        )
        ...
        print(
            "  3. Now includes intelligent retry with exponential backoff and user agent rotation."
        )
        print("------------")
            ...
            for date_format in date_formats:
                try:
                    ...
                    if "%Y" not in date_format:
                        parsed_date = datetime.strptime(time_str, date_format).replace(
                            year=now.year
                        )
                    else:
    ...
    def get_news(
        ...
        results = []
        total_articles_target = max_articles if max_articles is not None else None
        articles_collected_total = 0

        try:
            for keyword in keywords:
                if (
                    total_articles_target is not None
                    and articles_collected_total >= total_articles_target
                ):
                ...
                if total_articles_target is None:
                    remaining_articles = self.max_articles_per_keyword
                else:
                ...
                while articles_found_for_keyword < remaining_articles:
                    ...
                    results_per_request = self._get_results_per_request(
                        int(remaining_articles - articles_found_for_keyword)
                    )
                    ...
                    params = {
                        ...
                        "start": page
                        * 100,  # Google uses 100 as the base unit for pagination
                        "num": results_per_request,
                    }

                    try:
                        print(
                            f'Fetching page {page + 1} for keyword "{keyword}" ({articles_found_for_keyword}/{remaining_articles} articles found)'
                        )
                        ...
                        for article in articles:
                            if articles_found_for_keyword >= remaining_articles:
                                print(
                                    f"Reached target of {remaining_articles} articles for this keyword."
                                )
                                break
                            if (
                                total_articles_target is not None
                            ...
                            try:
                                ...
                                article_url = canonical_url or link
                                ...
                                target_label = (
                                    total_articles_target
                                    if total_articles_target is not None
                                    else "?"
                                )
                                ...
                                print(
                                    f"Found article {articles_collected_total}/{target_label}: {safe_title[:60]}..."
                                )
                        ...
                        print(
                            f'Found {articles_processed} articles for keyword "{keyword}" on page {page + 1}'
                        )
                        ...
                        if self.enable_graceful_degradation:
                            is_degraded = (
                                self.degradation_status.check_degradation_threshold(
                            ...
                            if is_degraded:
                                ...
                                if COLLECT_PARTIAL_RESULTS and results:
                                    print(
                                        f"Collecting partial results: {len(results)} articles retrieved so far"
                                    )
                                    self.degradation_status.collected_results_count = (
                                        len(results)
                                    )
                        ...
                        error_msg = (
                            f'Unexpected error for keyword "{keyword}": {str(e)}'
                        )
                        print(error_msg)
                        logger.error(error_msg)
                        self.degradation_status.update_failure(error_msg)
                ...
                if keyword != keywords[-1] and (
                    total_articles_target is None
                    or articles_collected_total < total_articles_target
                ):
                    delay = random.uniform(*RANDOM_DELAY_RANGE)
        ...
        if self.degradation_status.is_degraded:
            ...
            print(
                f"  - Successful Attempts: {self.degradation_status.successful_attempts}/{self.degradation_status.total_attempts}"
            )
            print(f"  - Failed Attempts: {self.degradation_status.failed_attempts}")
            print(
                f"  - Consecutive Failures: {self.degradation_status.consecutive_failures}"
            )
            if COLLECT_PARTIAL_RESULTS and results:
