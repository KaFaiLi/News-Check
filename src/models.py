...
class DegradationStatus(BaseModel):
    ...
    def check_degradation_threshold(self, min_success_threshold: float, max_consecutive_failures: int) -> bool:
        ...
        if self.success_rate < min_success_threshold or self.consecutive_failures >= max_consecutive_failures:
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [{
                ...
                "warnings": ["Failed to fetch 5 articles due to blocking", "Operating in degraded mode"]
            }]
        }
    }
